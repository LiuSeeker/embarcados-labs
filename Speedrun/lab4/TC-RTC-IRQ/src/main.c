...
void RTC_Handler(void)
{
	...
	*/
	...
	if ((ul_status & RTC_SR_ALARM) == RTC_SR_ALARM) {
		...
		if (sec >= 55){
			rtc_set_time_alarm(RTC, 1, hora, 1, min+1, 1, sec-55);
		}
		rtc_set_time_alarm(RTC, 1, hora, 1, min, 1, sec+5);
		if(flag_led0){
			tc_stop(TC0, 1);
			tc_stop(TC0, 2);
			tc_stop(TC1, 0);
...
int main(void){
	...
	TC_init(TC0, ID_TC1, 1, 20);
	TC_init(TC0, ID_TC2, 2, 8);
	TC_init(TC1, ID_TC3, 0, 11);
	TC_init(TC1, ID_TC4, 1, 17);
